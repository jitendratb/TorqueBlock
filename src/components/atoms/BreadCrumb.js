...
const Breadcrumb = ({ items = [], className = "", separator = <IoIosArrowForward size={16} />, }) => {
  ...
  return (
    <nav aria-label="Breadcrumb" className={`w-full ${className}`}>
      <ol className="flex flex-1 w-full items-center gap-2 overflow-x-auto whitespace-nowrap text-sm md:text-[15px] text-zinc-500 py-1">
        ...
        {items?.map((item, index) => {
          ...
          return (
            <li key={index} className="flex items-center gap-2 shrink-0" >
              {!isLast ? (
                <Link href={item.href || "#"} className="text-xs md:text-sm transition-all duration-200 text-white/70 hover:text-orange-500 max-w-[300px] md:max-w-[500px] truncate inline-block">
                  {formattedName}
                </Link>
              ) : (
                <span
                  className="text-white font-semibold truncate text-xs md:text-sm max-w-[300px] md:max-w-[500px] inline-block"
                >
                  {formattedName}
                </span>
              )}
              ...
              {!isLast && (
                <span className="text-zinc-400 shrink-0 select-none ml-1">
                  {separator}
                </span>
              )}
            </li>
