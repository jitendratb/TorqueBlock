...
function BrandCard({ brand }) {
  return (
    <Link href={`/search?q=${brand?.name}`} className='group block cursor-pointer relative h-[250px] md:h-[220px] lg:h-[240px] w-[280px] md:w-[320px] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden bg-zinc-900 shadow-xl w-full'>
      <Image
        src={brand?.brandBanner || brand?.brandLogo || brand?.heroImage || '/placeholder.png'}
      ...
      <div className='absolute bottom-0 left-0 w-full p-6 md:p-8 flex items-center justify-between z-10'>
        <div className='w-full'>
          ...
          <div className='flex justify-between items-end  w-full'>
            ...
            <div>
              <div className="w-10 h-10  flex-shrink-0 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-md border border-white/5 text-white/90 group-hover:bg-black/40 group-hover:text-white transition-all duration-500 ease-out">
