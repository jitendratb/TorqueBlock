...
import { FiArrowRight, FiCheckCircle, FiClock, FiXCircle, FiShoppingCart } from "react-icons/fi";
import useCartStore from '@/stores/cartStore';
import { useToast } from '@/context/ToastContext';

export default function TubeCard({ tube, className = '', addToCartStatus = false }) {
    const router = useRouter();
    const { addToCart } = useCartStore();
    const toast = useToast();

    const title = tube?.name || tube?.productName || tube?.hero?.title || "Premium Tube";
    const categoryName = tube?.brand?.name || tube?.brand || tube?.categoryId?.name || "Premium Tube";

    const rawPrice = tube?.pricing?.mrp || tube?.pricing?.price || tube?.pricing?.frontTyrePrice || tube?.basePrice || null;
    ...
    const displayPrice = rawPrice ? formatPrice(rawPrice) : "On Request";
    const labelText = "Price";

    // Default to in_stock if not provided
    const availability = tube?.availability || "in_stock";

    const sizeCountText = availability === "in_stock" ? "In Stock"
        : availability === "backorder" ? "Available To Order"
            : availability === "preorder" ? "Pre Order"
                : "Out of Stock";

    const handleCardClick = () => {
        const identifier = tube?.identifier || tube?._id || "unknown";
        router.push(`/tubes/${identifier}`);
    };
    ...
    const handleAddToCartClick = (e) => {
        ...
        addToCart(tube, null, null, genericTube);
    ...
    return (
        ...
        >
            ...
            <div className="relative w-full h-[180px] flex items-center justify-center p-4">
                ...
                <span className={`absolute top-4 right-4 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-extrabold uppercase tracking-wider backdrop-blur-md border transition-all duration-300 ${availability === "in_stock"
                ...
                <div className="flex items-end justify-between gap-2 pt-3 border-t border-white/10 transition-colors duration-500">
                    ...
                    {addToCartStatus ? (
                        <button
                            ...
                            className="flex items-center gap-1.5 text-xs font-black uppercase tracking-wider justify-center w-auto h-9 px-4 rounded-lg bg-orange-500 hover:bg-orange-600 text-white shadow-[0_0_15px_rgba(249,115,22,0.3)] transition-all duration-300 active:scale-95 cursor-pointer z-10"
                        >
