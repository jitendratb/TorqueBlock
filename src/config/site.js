export const siteConfig = {
  name: "Torque Block",
  shortName: "TorqueBlock",
  title: "Torque Block — Premium Performance Motorcycle Tyres in India",
  description: "India’s premium marketplace for performance motorcycle tyres. Compare, choose, and buy superbike tyres from Pirelli, Metzeler, Michelin, and more. Find the best tyres for grip, touring, street riding, and track performance.",

  keywords: [
    "performance motorcycle tyres",
    "superbike tyres India",
    "Pirelli tyres India",
    "Metzeler tyres India",
    "Michelin bike tyres",
    "best tyres for superbikes",
    "Ninja 650 tyres",
    "Z900 tyres",
    "BMW S1000RR tyres",
    "track tyres India",
    "touring tyres India",
    "sportbike tyres",
    "motorcycle tyre comparison",
    "premium bike tyres",
    "Metzeler ME 888 Marathon Ultra WW",
    "Michelin Road 6",
    "Michelin Anakee Adventure",
    "Michelin Scorcher 31",
    "Michelin Scorcher 11",
    "Michelin Power 6",
    "Michelin Road Classic",
    "Michelin Road 5",
    "Michelin Anakee Road",
    "Michelin Pilot Street Radial",
    "MICHELIN Anakee Adventure 2",
    "Michelin Anakee Wild",
    "Metzeler Roadtec 01",
    "Pirelli Diablo Rosso 2",
    "Pirelli Scorpion MX32 Mid Soft",
    "Pirelli MT21 Rallycross",
    "Michelin Commander 3",
    "Michelin ROAD W GT",
    "Metzeler Karoo 4 Street",
    "Michelin Power Cup 2",
    "Pirelli Angel Scooter",
    "Michelin Power GP 2",
    "Michelin Road 6 GT",
    "Pirelli Superbike SC1",
    "Pirelli Superbike SC2",
    "Pirelli Diablo Rosso Scooter",
    "Metzeler MCE 6 Days Extreme",
    "Apollo Alpha H1",
    "Vredestein Centauro ST",
    "Eurogrip Sport Radial"
  ],

  url: "https://www.torqueblock.com",
  logo: "/favicon.ico",
  ogImage: "/favicon.ico",
  email: "ops@torqueblock.com",
  phone: "+91-6366625625",
  socials: {
    instagram: "https://www.instagram.com/torque_block/",
    youtube: "https://www.youtube.com/@torqueblock",
    facebook: "https://www.facebook.com/torqueblock",
  },

  business: {
    type: "Automotive",
    country: "India",
    region: "Karnataka",
    city: "Bangalore",
  },

  categories: [
    "Track Tyres",
    "Sport Touring Tyres",
    "Hypersport Tyres",
    "Street Tyres",
    "Adventure Tyres",
    "Wet Grip Tyres",
    "Long Distance Touring Tyres",
  ],

  brands: [
    "Pirelli",
    "Metzeler",
    "Michelin",
    "Reise",
    "Maxxis",
    "Vredestein",
    "MRF",
    "Ceat",
    "Eurogrip",
    "Apollo"
  ],

  topBikePages: [
    "Kawasaki Ninja 650",
    "Kawasaki Z900",
    "BMW S1000RR",
    "Ducati Panigale V4",
    "Yamaha R1",
    "Suzuki Hayabusa",
    "Triumph Street Triple RS",
  ],

  seo: {
    titleTemplate: "%s | Torque Block",
    defaultTitle:
      "Premium Superbike Tyres in India | Torque Block",
    canonical: "https://www.torqueblock.com",
  },

  openGraph: {
    type: "website",
    locale: "en_IN",
    siteName: "Torque Block",
  },

  twitter: {
    card: "summary_large_image",
    creator: "@torqueblock",
  },

  analytics: {
    googleAnalyticsId: "G-L378GT1VFV",
    googleTagManagerId: "GTM-P8RVB5HS",
  },

  leadCapture: {
    whatsappEnabled: true,
    sleekflowEnabled: true,
    callbackEnabled: true,
  },

  trust: {
    authorisedDealers: true,
    genuineProducts: true,
    panIndiaShipping: true,
    expertSupport: true,
  },
};